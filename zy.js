...
//数据类型检测模块
        ...
        $group:function(content) {
            ...
            for(var i=0,len=arr.length;i<len;i++) {
                ...
                if(first === '.') {
                    ...
                    doms=[$$.$id(item.slice(index+1))]//之前我们定义的doms是数组，但是$id获取的不是数组，而是单个元素
                    //封装重复的代码成函数
                    pushArray(doms,result)
                }else{
                    doms = $$.$tag(item)
    ...
    Animate.prototype = {
        ...
        //新增一个loop以此针对每个物体做运动 --其实就是遍历每个对象，然后依次执行move方法
        loop:function(){
